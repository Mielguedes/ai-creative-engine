...
def buscar_perfil(access_token, user_id):
    """Busca o perfil usando a estrutura atual da tabela profiles do Loop de Live."""
...
def restaurar_projeto_do_storage():
    """Restaura os vídeos do projeto a partir do manifesto no Supabase Storage."""
    ...
    manifest_url = (
        f"{SUPABASE_URL}/storage/v1/object/{st.secrets.get('SUPABASE_STORAGE_BUCKET', 'ai-creative-engine')}"
        f"/{manifest_object}"
    )
    headers = {
        "apikey": SUPABASE_KEY,
        "Authorization": f"Bearer {st.session_state.get('access_token') or SUPABASE_KEY}",
    }

    try:
        resposta = requests.get(manifest_url, headers=headers, timeout=30)
        # Um projeto novo ainda não possui manifesto.
        # O Storage pode responder 400 ou 404 nessa consulta; isso não deve
        # bloquear a tela nem impedir novos uploads.
...
def salvar_arquivos(files, destino, storage_subfolder):
    ...
    # Os arquivos são enviados individualmente por save_uploaded_files.
    # Não reenviaremos todo o projeto a cada seleção, evitando travamentos e
    # falhas em cascata no Streamlit. O manifesto será tratado separadamente.
    return salvos
...
with col1:
    ...
    arquivos_h = [f for f in os.listdir(PATH_GANCHOS) if f.lower().endswith(('.mp4', '.mov', '.m4v'))]
...
with col2:
    ...
    arquivos_m = [f for f in os.listdir(PATH_CORPOS) if f.lower().endswith(('.mp4', '.mov', '.m4v'))]
...
with col3:
    ...
    arquivos_c = [f for f in os.listdir(PATH_CTAS) if f.lower().endswith(('.mp4', '.mov', '.m4v'))]
...
if st.button("🚀 Multiplicar e Gerar Todos os Vídeos", type="primary", use_container_width=True):
    list_h = [os.path.join(PATH_GANCHOS, f) for f in os.listdir(PATH_GANCHOS) if f.lower().endswith(('.mp4', '.mov', '.m4v'))]
    list_m = [os.path.join(PATH_CORPOS, f) for f in os.listdir(PATH_CORPOS) if f.lower().endswith(('.mp4', '.mov', '.m4v'))]
    list_c = [os.path.join(PATH_CTAS, f) for f in os.listdir(PATH_CTAS) if f.lower().endswith(('.mp4', '.mov', '.m4v'))]
