...
if not SUPABASE_URL or not SUPABASE_KEY:
    ...
    st.info(
        "Verifique os Secrets do Streamlit e confirme que existem "
...
"""
        ...
        for segment in segments:
            ...
            for grupo in grupos:
                ...
                for i_destaque, w_foco in enumerate(
                    grupo
                ):
                    ...
                    for j_palavra, w_item in enumerate(
                        grupo
                    ):
...
# ============================================================
# FUNÇÃO - PROCESSAR BLOCO INDIVIDUAL
# ============================================================
...
st.caption(
    "Multiplicador Modular de Vídeos Localhost "
    "(Estável & Seguro)"
)
...
if total_variacoes > 0:
    ...
    st.info(
        ...
        f"**{total_variacoes} Vídeo(s) Resultante(s)**!"
    )
...
tipo_gpu = st.selectbox(
    "Escolha a Renderização "
    "(Se congelar no seu PC, troque para CPU Padrão):",
    [
        "CPU Padrão (libx264) - Mais Estável",
        "NVIDIA (h264_nvenc)",
        "AMD (h264_amf)",
        "Intel (h264_qsv)"
    ]
...
st.markdown(
    "#### 🛡️ Modificadores Anti-Duplicidade "
    "Agressivos (Sem Alterar Voz)"
)
...
with col_e1:
    ...
    auto_ultra_anti_dup = st.checkbox(
        "Modo Ultra Anti-Duplicidade "
        "(Brilho/Contraste, Pan/Crop Estável)",
        value=True
    )
...
with col_e2:
    ...
    espelhar_blocos_rand = st.checkbox(
...
st.markdown(
    "#### 📌 Hooks Alternativos "
    "(Texto Inclinado no topo)"
)

hook_ativo = st.checkbox(
    "Ativar Hook no topo do vídeo",
    value=True
)
...
if hook_ativo:
    ...
    col_h1, col_h2, col_h3 = st.columns(
        [2, 1, 1]
    )
    ...
    with col_h1:
        ...
        texto_manchete = st.text_area(
            "Escreva frases de Hook "
            "(Uma frase por linha para ser usada nas variações):",
            value="",
            placeholder=(
                "Digite suas frases aqui "
                "(uma por linha)..."
            ),
            height=120
        )
    ...
    with col_h2:
        ...
        posicao_hook_y = st.number_input(
            "Posição do Hook (px do topo):",
            min_value=10,
            max_value=1200,
            value=200,
            step=10
...
st.markdown(
    ...
    "(Word Pop Zoom + Destaque Amarelo)"
)

legenda_ativa = st.checkbox(
    "Ativar Legendas Automáticas no Vídeo",
    value=True
)
...
if legenda_ativa:
    ...
    with col_l1:
        ...
        posicao_legenda_v = st.slider(
            "Altura da Legenda na Tela "
            "(px a partir de baixo):",
            min_value=100,
            max_value=1200,
            value=450,
            step=20
...
if st.button(
    ...
    if (
        ...
        st.error(
            ...
            "em cada uma das 3 colunas para gerar!"
        )
        ...
        st.success(
            f"🔥 Gerando {len(combos)} vídeo(s) "
            f"com Encoder [{encoder_escolhido}]..."
        )

        prog = st.progress(0)

        for idx, (h, m, c) in enumerate(
            combos
        ):

            print(
                f"\n=================== "
                f"PROCESSANDO VARIAÇÃO "
                f"{idx+1}/{len(combos)} "
                f"==================="
            )
            ...
            if not os.path.exists(
                ...
                st.error(
                    f"Erro ao concatenar "
                    f"variação {idx+1}. "
                    f"Mude para 'CPU Padrão' nas opções!"
                )
            ...
            if auto_ultra_anti_dup:
                ...
                cmd_filt = (
                    ...
                    f'-metadata title="" '
                    f'-metadata comment="" '
                    f'-metadata encoder="Apple iOS Camera 17.4.1" '
                    f'"{temp_filt}"'
                )


                print(
                    "Aplicando Modulação Visual "
                    "Anti-Duplicidade: "
                    f"{cmd_filt}"
                )
...
if not videos_gerados:
    ...
    st.info(
        ...
        "Suba os arquivos nas colunas acima "
        "e clique no botão para gerar!"
    )
    ...
    zip_path = os.path.join(
        PROJ_PATH,
        f"{projeto_atual}_todos_os_videos.zip"
    )
    ...
    score_geral = calcular_score_local(
        texto_manchete
    )


    for idx, vid_file in enumerate(
        videos_gerados
    ):
