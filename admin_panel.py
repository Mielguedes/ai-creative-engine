...
def cadastrar_acesso(access_token: str, email: str, plan: str) -> None:
    """Cadastra ou atualiza um acesso ativo por e-mail."""
    url, _ = _config()
...
def render_admin_panel(access_token: str, is_admin: bool) -> None:
    ...
    with st.expander("🛡️ Administração de membros", expanded=False):
        st.caption("Cadastre o membro informando apenas o e-mail e o plano. O acesso será ativado automaticamente e a validade será calculada pelo sistema.")

        st.markdown("### ➕ Novo membro")
        with st.form("new_access_form", clear_on_submit=True):
            new_email = st.text_input("E-mail", placeholder="pessoa@email.com")
            ...
            for row in rows:
                ...
                with st.expander(email or "Membro sem e-mail", expanded=False):
                    ...
                    plan = st.selectbox(
                        "Plano",
                        PLANOS,
                        index=PLANOS.index(normalizar_plano(row.get("plan"))),
                        format_func=lambda value: PLANOS_LABELS[value],
                        key=f"access_plan_{identifier}",
                    )
                    expires_at = st.text_input(
                        "Validade (opcional)",
                        value=_normalizar_validade(row.get("expires_at")),
